...
# endif
	...
	if (m_bActive)
	{
		const auto  borderColor = Editor->GetColor(StyleColor_ActiveNodeBorder);
		const auto& editorStyle = Editor->GetStyle();

		drawList->ChannelsSetCurrent(m_Channel + c_NodeBaseChannel);

		DrawBorder(drawList, borderColor, editorStyle.ActiveNodeBorderWidth);
	}
}
...
void ed::NodeBuilder::End()
{
	...
	//EX Add by Fei
	{
		m_CurrentNode->m_bActive = m_bActive;
	}
...
void ed::NodeBuilder::NodeSetActive(bool bActive)
{
	IM_ASSERT(nullptr != m_CurrentNode);
	m_bActive = bActive;
}
