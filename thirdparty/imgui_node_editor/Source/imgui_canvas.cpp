...
void ImGuiEx::Canvas::Suspend()
{
    ...
    if (m_SuspendCounter == 0 && m_bVisable)
        LeaveLocalSpace();
...
void ImGuiEx::Canvas::SaveInputState()
{
	...
	for (auto i = 0; i < IM_ARRAYSIZE(m_MouseClickedPosBackup); ++i)
	{
		...
		LOG_INFO("liufei mouse SAVE:(%f,%f)", io.MouseClickedPos[0].x, io.MouseClickedPos[0].y);
	}
        
    // Record cursor max to prevent scrollbars from appearing.
    m_WindowCursorMaxBackup = ImGui::GetCurrentWindow()->DC.CursorMaxPos;
	m_MouseViewPosBackup = ImGui::GetMouseViewport()->Pos;
	m_MouseViewSizeBackup = ImGui::GetMouseViewport()->Size;
	m_WindowViewPosBackup = ImGui::GetWindowViewport()->Pos;
	m_WindowViewSizeBackup = ImGui::GetWindowViewport()->Size;
}
...
void ImGuiEx::Canvas::RestoreInputState()
{
    ...
    for (auto i = 0; i < IM_ARRAYSIZE(m_MouseClickedPosBackup); ++i)
	{
		...
		LOG_INFO("liufei mouse RESTORE:(%f,%f)", io.MouseClickedPos[0].x, io.MouseClickedPos[0].y);
	}
        
    ImGui::GetCurrentWindow()->DC.CursorMaxPos = m_WindowCursorMaxBackup;
	ImGui::GetWindowViewport()->Pos = m_WindowViewPosBackup;
	ImGui::GetWindowViewport()->Size = m_WindowViewSizeBackup;
	ImGui::GetMouseViewport()->Pos = m_MouseViewPosBackup;
	ImGui::GetMouseViewport()->Size = m_MouseViewSizeBackup;
}
...
# endif
	...
	for (auto i = 0; i < IM_ARRAYSIZE(m_MouseClickedPosBackup); ++i)
	{
		if (i == 0)
		{
			LOG_INFO("liufei mouse change before:(%f,%f)", io.MouseClickedPos[0].x, io.MouseClickedPos[0].y);
		}
		...
		if (i == 0)
		{
			LOG_INFO("liufei mouse change after:(%f,%f)", io.MouseClickedPos[0].x, io.MouseClickedPos[0].y);
		}
	}
	...
	//Set Imgui View for NodeEdit
	ImGui::GetWindowViewport()->Pos = { m_ViewRect.Min.x, m_ViewRect.Min.y };
	ImGui::GetWindowViewport()->Size = { m_ViewRect.Max.x - m_ViewRect.Min.x, m_ViewRect.Max.y - m_ViewRect.Min.y };
	ImGui::GetMouseViewport()->Pos = { m_ViewRect.Min.x, m_ViewRect.Min.y };
	ImGui::GetMouseViewport()->Size = { m_ViewRect.Max.x - m_ViewRect.Min.x, m_ViewRect.Max.y - m_ViewRect.Min.y };
